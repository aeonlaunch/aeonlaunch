...
import { CHAIN, REVENUE_MANAGER_ADDRESS, WORKER_API_URL } from "../lib/config.js";
  ...
  try {
    ...
    if (!json) {
      if (isNew) {
        ...
        console.log(`Key saved to ~/.aeon/wallet.json (never share this file)\n`);
      } else {
    ...
    // Register token and trigger indexer (both production and local dev worker)
    const workerUrls = Array.from(new Set([WORKER_API_URL, "http://localhost:8787"]));
    if (!json) process.stdout.write("Registering with network indexer...");
    await Promise.allSettled(workerUrls.map((base) =>
      fetch(`${base}/api/tokens/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tokenAddress, symbol, name, description }),
      })
    ));
    await Promise.allSettled(workerUrls.map((base) =>
      fetch(`${base}/api/network/trigger`)
    ));
    if (!json) console.log(" done");
    ...
    if (isNew) {
      outputData.walletPath = "~/.aeon/wallet.json";
