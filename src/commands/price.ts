...
export async function price(opts: PriceOpts): Promise<void> {
  ...
  try {
    ...
    const marketCapETH = details.marketCapETH;
    const volume24hETH = details.volume24hETH;
    ...
    if (json) {
      const output: Record<string, unknown> = {
        ...
        createdAt: details.createdAt ? new Date(details.createdAt * 1000).toISOString() : null,
        flaunchUrl,
        network: chain.name,
      };
    ...
    const changeStr = formatChange(details.priceChange24h);
