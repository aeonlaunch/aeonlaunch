...
/** Fetch all aeonlaunch tokens via V2 royalties endpoint (fallback) */
async function fetchAllTokensFallback(): Promise<RoyaltyCoin[]> {
  const url = `${FLAUNCH_DATA_API_BASE}/v2/base/users/${REVENUE_MANAGER_ADDRESS}/royalties`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Flaunch API error: ${res.status}`);
  const json = (await res.json()) as RoyaltiesResponse;
  return json.coins ?? [];
}
...
function renderRichOutput(
  ...
  console.log(`\nthe aeonlaunch network — ${agents.length} agent(s)\n`);
...
async function renderFallback(tokens: RoyaltyCoin[], json: boolean): Promise<void> {
  const agents: NetworkAgent[] = tokens.map((t) => {
    const mcapUsd = parseFloat(t.marketCapUSD || "0");
    const feesEth = parseFloat(t.feesEarnedETH || "0");
    return {
      tokenAddress: t.tokenAddress,
      name: t.name || "unnamed",
      symbol: t.symbol || "???",
      creator: "—",
      marketCapETH: mcapUsd > 0 ? `$${mcapUsd.toLocaleString("en-US", { maximumFractionDigits: 2 })}` : "$0",
      claimableETH: feesEth > 0 ? formatEth(feesEth) + " earned" : "0 ETH",
      image: t.image || "",
    };
  });
  ...
  console.log(`\nthe aeonlaunch network — ${agents.length} agent(s) (basic mode)\n`);
  ...
  console.log(`${agents.length} agent(s) on the aeonlaunch network\n`);
}
  ...
  try {
    ...
    if (tokens.length === 0) {
      if (json) {
        ...
        console.log("No agents found. Be the first: npx anl launch\n");
      }
      return;
    }

    if (!json) console.log(`Found ${tokens.length} agent(s).\n`);
