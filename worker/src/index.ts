import type { Env, NetworkState, NetworkGoal, FlaunchListToken } from './types';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // POST /api/tokens/register — CLI calls this after every launch to index the token
    if (request.method === 'POST' && url.pathname === '/api/tokens/register') {
      const body = await request.json() as Partial<FlaunchListToken & { creator?: string; launchedAt?: string }>;
      if (!body.tokenAddress || !body.symbol || !body.name) {
        return jsonResponse({ error: 'tokenAddress, symbol, and name are required' }, 400, corsHeaders);
      }

      const raw = await env.NETWORK_KV.get(REGISTERED_TOKENS_KEY);
      const existing: FlaunchListToken[] = raw ? JSON.parse(raw) : [];

      const addrLower = body.tokenAddress.toLowerCase();
      const alreadyRegistered = existing.some((t) => t.tokenAddress.toLowerCase() === addrLower);
      ...
      if (!alreadyRegistered) {
        ...
        await env.NETWORK_KV.put(REGISTERED_TOKENS_KEY, JSON.stringify(existing));
        // Kick off a pipeline refresh so the new token appears immediately
        ctx.waitUntil(runPipeline(env));
        return jsonResponse({ status: 'registered', tokenAddress: body.tokenAddress, pipelineTriggered: true }, 200, corsHeaders);
      }

      return jsonResponse({ status: 'already_registered', tokenAddress: body.tokenAddress }, 200, corsHeaders);
    }
    ...
    // GET /api/tokens — list all registered (CLI-launched) tokens
    if (url.pathname === '/api/tokens') {
      const raw = await env.NETWORK_KV.get(REGISTERED_TOKENS_KEY);
      const tokens: FlaunchListToken[] = raw ? JSON.parse(raw) : [];
