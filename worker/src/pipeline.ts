...
import { fetchTokens, fetchTokenDetails, fetchHolders, fetchSwaps, fetchDexScreenerMarketData, batchedFetch } from './flaunch';
import { batchReadBalances, batchFetchMemos, fetchWalletSwaps, fetchTokenImage } from './rpc';
...
async function executePipeline(env: Env): Promise<void> {
  ...
  // 1b. Pre-fetch DexScreener market data for all tokens in one batch call
  const dexData = await fetchDexScreenerMarketData(tokens.map((t) => t.tokenAddress));
  console.log(`[pipeline] dexscreener: ${dexData.size}/${tokens.length} tokens with live market data`);
  ...
  // All royalties tokens are active — skip market cap pre-filter
  const candidates = tokens;
  console.log(`[pipeline] ${candidates.length} candidates (royalties-based)`);
  ...
  // 3. Enrich qualified tokens (details + swaps + on-chain image)
  ...
  await batchedFetch(qualified, async (t) => {
    ...
    const [details, swaps] = await Promise.all([
      fetchTokenDetails(env, t.tokenAddress, dexData),
      fetchSwaps(env, t.tokenAddress),
    ]);

    // Resolve image from the ERC20 tokenURI() if not already set
    if (details && !details.image) {
      details.image = await fetchTokenImage(env, t.tokenAddress);
    }
    // Also backfill on the list token so the agent object gets it
    if (!t.image) {
      t.image = (details?.image) ?? await fetchTokenImage(env, t.tokenAddress);
    }
  ...
  for (const t of qualified) {
    ...
    const owner = enrich?.details?.creator ?? '';
    if (owner) {
  ...
  const allAgents: Agent[] = qualified.map((t) => {
    ...
    return {
      ...
      symbol: t.symbol || '???',
      creator: owner,
      marketCapETH: parseFloat(details?.marketCapETH || '0'),
      volume24hETH: parseFloat(details?.volume24hETH || '0'),
      priceChange24h: parseFloat(details?.priceChange24h || '0'),
